...
from typing import AsyncGenerator, Dict, Optional, Tuple, Union, cast
...
class AsyncWebsocketClient(RawAsyncWebsocketClient):
    """

    The main class for interacting with the Async Home Assistant WebSocket API client.

    Here's a quick example of how to use the :py:class:`AsyncWebsocketClient` class:
    ...
    .. code-block:: python
        ...
        from homeassistant_api import AsyncWebsocketClient
            ...
            async with AsyncWebsocketClient(
                ...
                light = await ws_client.trigger_service('light', 'turn_on', entity_id="light.living_room")
    """
    ...
    def __init__(
        ...
        logger.debug(f"AsyncWebsocketClient initialized with api_url: {api_url}")
    ...
    async def get_rendered_template(self, template: str) -> str:
        ...
        """
        id = await self.send("render_template", template=template, report_errors=True)
        first = await self.recv(id)
        assert cast(ResultResponse, first).result is None
        second = await self.recv(id)
    ...
    async def get_config(self) -> dict[str, JSONType]:
        ...
        return cast(
            ...
            cast(
                ResultResponse,
                await self.recv(await self.send("get_config")),
            ).result,
        )
    ...
    async def get_states(self) -> Tuple[State, ...]:
        ...
        return tuple(
            ...
            for state in cast(
                ...
                cast(
                    ResultResponse, await self.recv(await self.send("get_states"))
                ).result,
            )
        )
    ...
    async def get_domains(self) -> dict[str, Domain]:
        ...
        """
        resp = await self.recv(await self.send("get_services"))
        domains = map(
    ...
    async def get_domain(self, domain: str) -> Domain:
        ...
        """
        return (await self.get_domains())[domain]
        ...
        .. code-block:: python
            ...
            async with ws_client.listen_events("test_event") as events:
                async for i, event in zip(range(2), events):  # to only wait for two events to be received
                    print(event)
        """
        subscription = await self._subscribe_events(event_type)
        yield cast(AsyncGenerator[FiredEvent, None], self._wait_for(subscription))
        await self._unsubscribe(subscription)

    async def _subscribe_events(self, event_type: Optional[str]) -> int:
        """
    ...
    async def listen_trigger(
        ...
        """
        subscription = await self._subscribe_trigger(trigger, **trigger_fields)
        yield (
    ...
    async def _subscribe_trigger(self, trigger: str, **trigger_fields) -> int:
        """
    ...
    async def _wait_for(
        ...
        while True:
            yield cast(
                ...
                cast(EventResponse, await self.recv(subscription_id)).event,
            )

    async def _unsubscribe(self, subcription_id: int) -> None:
        """
    ...
    async def fire_event(self, event_type: str, **event_data) -> Context:
        """
        Fire an event.
