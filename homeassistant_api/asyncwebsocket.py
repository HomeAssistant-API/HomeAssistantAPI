...
class AsyncWebsocketClient(BaseWebsocketClient):
    ...
    async def get_states(self) -> Tuple[State, ...]:
        ...
        return tuple(
            ...
            for state in cast(
                ...
                cast(
                    ResultResponse,
                    await self.recv(await self.send("get_states")),
                ).result,
            )
        )
    ...
    async def get_domains(self) -> dict[str, Domain]:
        ...
        domains = map(
            lambda item: Domain.from_json_with_client(
                ...
                client=cast("AsyncWebsocketClient", self),
            ),
    ...
    async def get_config_entries(self) -> Tuple[ConfigEntry, ...]:
        ...
        """
        resp = await self.recv(await self.send("config_entries/get"))
        return tuple(
    ...
    async def get_nonuser_flows_in_progress(self) -> Tuple[FlowResult, ...]:
        ...
        """
        resp = await self.recv(await self.send("config_entries/flow/progress"))
        return tuple(
    ...
    async def listen_config_entries(
        ...
        """
        subscription = (await self.recv(await self.send("config_entries/subscribe"))).id
    ...
    async def _async_wait_for_config_entries(
        ...
        while True:
            event_resp = cast(EventResponse, await self.recv(subscription_id))
